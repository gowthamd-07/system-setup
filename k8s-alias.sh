...
## get 'real' all
alias kgworld='kubectl api-resources --verbs=list --namespaced -o name | tr "\n" "," | sed "s/,$//" | xargs kubectl get'
...
kei() {
  ...
  kubectl exec -it -n $namespace $pod_name -- sh -c '
    ...
    command -v ash  >/dev/null 2>&1 && exec ash  ||
    command -v sh  >/dev/null 2>&1 && exec sh || 
...
ky() {
  ...
  if [[ -z "$1" ]]; then
    ...
    echo "                                Otherwise, saves as <namespace><resource><name>.yaml."
    echo "Examples:"
    echo "  ky deployment"
    echo "  ky pod yes"
  ...
  echo "$selected_items" | while IFS= read -r line; do
    ...
    if [[ "$omit_details_in_filename" == "true" || "$omit_details_in_filename" == "yes" || "$omit_details_in_filename" == "y" ]]; then
      ...
      filename="${namespace}${resource_name}${sanitized_name}.yaml"
    fi
